...
const uri = `mongodb+srv://davinci2003:${
  process.env.MONGODB ? process.env.MONGODB : 'Rje3zA4rFbhLmtop'
}@cluster0.x5mhjao.mongodb.net/?retryWrites=true&w=majority`;
...
app.get(
  ...
  async (req, res, next) => {
    ...
    const thisWeek =
      Math.floor((Math.floor(Date.now() / 1000) - 1705244400) / 604800) + 1;
      ...
      try {
        ...
        if (result) {
          ...
          while (true) {
            ...
            const submitPage = d.map((ele) => {
              try {
                ...
                const submitResult =
                  ele.children[3].children[0].attribs['data-color'];
                const submitTime =
                  ele.children[8].children[0].attribs['data-timestamp'];
                return {
                  submitID: submitID,
                  problemID: problemID,
          ...
          res.data = res.data.filter(
            (ele) => ele.submitTime >= s && ele.submitTime <= e,
          );
